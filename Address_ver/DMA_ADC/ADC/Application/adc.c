...
void Init_DMA(void) //DMA memory address: 0x40020000
{
	(*(volatile unsigned int*)0X40021014)=0X01;// RCC->AHBENR=0X01; << DMA1 Enable
	
	(*(volatile unsigned int*)0x4002000C)=0X02;// DMA1_Channel1->CNDTR=0X02; << DMA1 CHANNEL1 transmit 2WORD
	(*(volatile unsigned int*)0x40020010)=0X20002000;// DMA1_Channel1->CPAR=0X20002000; <<DMA1 channel1 prepheral address
	(*(volatile unsigned int*)0x40020014)=0X2000201F;// DMA1_Channel1->CMAR=0X2000201F; <<DMA1 CHANNEL1 memory address
	
	(*(volatile unsigned int*)0x40020008)=(0x01<<5);// DMA1_Channel1->CCR=(0x01<<5); << circular(loop)
	(*(volatile unsigned int*)0x40020008)|=(0x01<<6);// DMA1_Channel1->CCR|=(0x01<<6); << Peripheral increment enable
	(*(volatile unsigned int*)0x40020008)|=(0x01<<7);// DMA1_Channel1->CCR|=(0x01<<7); << Memory increment enable
	(*(volatile unsigned int*)0x40020008)|=(0x01<<14);// DMA1_Channel1->CCR|=(0x01<<14); << Memory to Memory
	
	(*(volatile unsigned int*)0x40020008)|=(0x01);// DMA1_Channel1->CCR|=(0x01); << DMA1 CHANNEL1 ENABLE
}
/*----------------------------------------------------------*\
 | MIAN ENTRY                                               |
\*----------------------------------------------------------*/
int main (void) {
  ...
  //adc_Init();
	...
	for(;;) {
	(*(volatile unsigned int*)0x20002000)=0xFF;
