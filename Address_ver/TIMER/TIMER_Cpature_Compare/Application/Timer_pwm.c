/*
LED	PWM.
LED0(PA.0) Cycle is 2ms, 0~100% duty control
*/
#include <stdio.h>
...
/*timer 1*/
	...
	(*(volatile unsigned int*)0x40012C28)=(0xE09);// TIM1->PSC=(0xE10); << TIMER1 Prescale is 3600
	(*(volatile unsigned int*)0x40012C2c)=(0x63);// TIM1->ARR=(0x63); << TIMER1 Interrupt count is 100(100번 count되면 업데이트 인터럽트 발생)
	...
	(*(volatile unsigned int*)0x40012C34)=START_DUTY;// TIM1->CCR1=START_DUTY; << Compare number set
...
void TIM2_IRQHandler (void) {
  ...
  if ((TIM2->SR & 0x0001) != 0) {                 // check interrupt source
		if( (*(volatile unsigned int*)0X40012C34)>98 || (*(volatile unsigned int*)0X40012C34)<=0)// if(TIM1->CCR1>=0X270F || TIM1->CCR1<=0)
		{
			g_dutyFlag*=-1;
		}
